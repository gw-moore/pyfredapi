...
def test_init():
    sc = SeriesCollection(client)
    assert isinstance(sc, SeriesCollection)
    assert isinstance(sc.client, FredSeries)
...
def test_keep_realtime_cols():
    ...
    df_cols = set(sc.CPIAUCSL.data.columns.to_list())
...
def test_rename_partial():
    ...
    sc.rename_series(rename=rename)
    assert set(("date", "cpi_all_items")) == set(sc.CPIAUCSL.data.columns.to_list())
    assert set(("date", "CPILFESL")) == set(sc.CPILFESL.data.columns.to_list())
